document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector(".content-body").addEventListener("click",e=>{const target=e.target;if(target.classList.contains("edit-btn")){openModal(target.dataset.type==="student"?"editStudent":"editCourse",JSON.parse(target.dataset.item))}if(target.classList.contains("delete-btn")){const{type,id}=target.dataset;if(confirm(`Are you sure you want to delete this ${type}?`)){const url=type==="student"?`/api/users/${id}`:`/api/courses/${id}`;fetch("https://unitrack-backend-8oyb.onrender.com"+url,{method:"DELETE",headers:{Authorization:`Bearer ${token}`}}).then(fetchAllData)}}});
    modalForm.addEventListener("submit",async e=>{e.preventDefault();const{type,id}=e.target.dataset;const formData=new FormData(e.target);const body=Object.fromEntries(formData.entries());let url,method;if(type==="addStudent"){url="https://unitrack-backend-8oyb.onrender.com/api/users/register";method="POST"}else if(type==="addCourse"){url="https://unitrack-backend-8oyb.onrender.com/api/courses";method="POST"}else if(type==="editStudent"){url=`https://unitrack-backend-8oyb.onrender.com/api/users/${id}`;method="PUT";if(!body.password)delete body.password}else if(type==="editCourse"){url=`https://unitrack-backend-8oyb.onrender.com/api/courses/${id}`;method="PUT"}try{const response=await fetch(url,{method,headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify(body)});if(!response.ok)throw new Error("Operation failed.");modal.style.display="none";fetchAllData()}catch(error){console.error("Form submission error:",error)}});
